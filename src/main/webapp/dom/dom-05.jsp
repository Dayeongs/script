	...
	for (Status s : statusList) {
		<div class="form-check">
			<input type="radio" 
			       ...
			       id="status-<%=s.getNo() %>"
			       value="<%=s.getNo() %>" />
			<lable class="form-check-label><%=s.getName() %></label>
		</div>	
	}
--%>		
			<form>
				...
				<div class="form-group">
					<label class="form-lavel">이름</label>
					<input type="text" class="form-control" name="username" />
				</div>
				<div class="form-group">
					<label class="form-lavel">닉네임</label>
				...
				<div class="form-group">
					<label class="form-lavel">상태</label>
					<div class="form-check">
				...
				<div class="form-group">
					<label class="form-lavel">이벤트</label>
					<div class="form-check">
...
<script type="text/javascript">
	...
	*/
	...
	// document.getElementById();	// Id가 없다.
	// document.getElementsByTagName(); // 여러개
	// document.getElementsByClassName(); // 여러개
	// document.querySelectorAll(); // 여러개
	let el1 = document.querySelector("select[name='catNo']"); // select와 대괄호 사이에 띄어쓰기가 없어야한다.
	...
	// text 입력필드의 값 조회하기
	// 값을 조회할때는 구체적으로 적기 - textarea로 조회하면 여러개 조회되므로 name까지 적어서 조회하기
	let el4 = document.querySelector("textarea[name='memo']");
	let value4 = el4.value;
	console.log(value4);
	
	// radio버튼 중에서 특정 라디오버튼을 체크하기
	// checked는 radio와 checkbox에만 있다.
	// radio버튼은 항상 1개만 체크된다. querySelector() 사용
