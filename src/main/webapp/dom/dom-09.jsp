	...
	<div class="row">
		<div class="col-12">
			...
			<!-- 
				...
				form 태그의 submit 이벤트에 대한 기본동작
					- submit 이벤트가 발생하면 폼 입력값을 서버로 제출한다.
			 -->
			
			<h5>form 태그의 submit 이벤트에 대한 기본동작 막기</h5>
...
<script type="text/javascript">
	...
	/*
		...
		// 2. 조회된 엘리먼트의 클래스 속성값을 가지고 있는 객체 조회하기
		let list = element.classList;
		* element.classList 프로퍼티에는 DOMTokenList 객체가 담겨있다.
		* DOMTokenList 객체는 문자열을 여러 개 저장하고 있는 객체다. (배열 유사 객체다.)
		* DOMTokenList 객체의 주요 메소드
			add("텍스트")
				지정된 텍스트를 DOMTokenList객체에 추가한다.
			remove("텍스트")
				지정된 텍스트를 DOMTokenList객체에서 삭제한다.
			toggle("텍스트")
				지정된 텍스트를 DOMTokenList객체에서 토글시킨다.
				존재하면 삭제하고, 없으면 추가한다.
			contains("텍스트")
				DOMTokenList객체에 지정된 텍스트가 존재하면 true,
	...
	*/
	...
	function checkForm(event) {
		...
		// 3. 입력필드의 값을 체크해서 경고창을 표시하거나, 폼입력값을 서버로 제출시킨다.
		if (keyword === "") {
			
			// 4. 경고메세지 엘리먼트를 선택한다.
			let div = document.getElementById("div-keyword-alert");
			
			// 5. class 속성값에서 "d-none"을 삭제한다.
			let list = div.classList;
			list.remove("d-none");
			
			// submit 이벤트가 발생했을 때 form 입력값이 서버로 제출되는 것을 막는다.
